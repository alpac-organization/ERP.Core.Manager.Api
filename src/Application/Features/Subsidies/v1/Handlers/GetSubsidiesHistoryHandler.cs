...
{
    public class GetSubsidiesHistoryHandler(IUnitOfWork _unitOfWork, IErrorManager _errorManager, ILogger<GetSubsidiesHistoryHandler> _logger)
        : AlpacBaseHandler<GetSubsidiesHistoryQuery, PagedResponse<SubsidyHistoryDto>>(_unitOfWork, _errorManager)
    {
        public override async Task<PagedResponse<SubsidyHistoryDto>> Handle(GetSubsidiesHistoryQuery request, CancellationToken cancellationToken)
        {
            _logger.LogInformation("Consultando historial de subsidios para la nómina: {PayrollId}", request.PayrollId);
            ...
            var query = _unitOfWork.Subsidies.Entities
                ...
                .Where(s => s.PayrollId == request.PayrollId);
